...
public class BeetleConnectionFactory extends ConnectionFactory {
  ...
  private Supplier<RecoverableConnection> connection(ExecutorService executor,
      AddressResolver resolver, String clientProvidedName) {
    return () -> (RecoverableConnection) super.newConnection(executor, resolver,
        clientProvidedName);
  }
  ...
  public Connection newConnection(ExecutorService executor, AddressResolver addressResolver,
    ...
    RetryExecutor retryExecutor =
        executor != null ? connectionEstablishingExecutor.withExecutor(executor)
            : connectionEstablishingExecutor;
    List<Connection> connections =
        addressResolver.getAddresses().stream().map(Collections::singletonList)
            .map(ListAddressResolver::new).map(res -> connection(executor, res, clientProvidedName))
            .map(retryExecutor::supply).map(RetryableConnection::new).collect(Collectors.toList());
