  ...
  public void run() {
    try {
      ...
      if (!currentMaster.equals(masterInFile)) {
        log.info(
            "Redis master configuration file changed: " + currentMaster + " -> " + masterInFile);
        currentMaster = masterInFile;

        final String[] masterHostPort = masterInFile.split(":");
        if (masterHostPort.length != 2) {
          log.warn(
              "Invalid file content '{}' in redis master file {}. Not performing master switch.",
              masterInFile,
              getMasterFile());
        } else {
          client
              .getDeduplicationStore()
              .reconnect(
                  new RedisConfiguration(masterHostPort[0], Integer.valueOf(masterHostPort[1])));
        }
      }
