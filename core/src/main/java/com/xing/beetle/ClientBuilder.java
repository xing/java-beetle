...
public class ClientBuilder {
  ...
  public ClientBuilder addBroker(
      String host, int port, String username, String password, String virtualHost)
      throws URISyntaxException {
    // The virtualHost has to start with a / or you would get a URISyntaxException. This is not
    // obvious and we avoid common problems here.
    if (virtualHost != null && !virtualHost.startsWith("/")) {
      virtualHost = "/" + virtualHost;
    }

    return addBroker(
        new URI("amqp", username + ":" + password, host, port, virtualHost, null, null));
  }
  ...
  public Client build() {
    ...
    if (executorService == null) {
      ...
      final ThreadFactory messageHandlerThreadFactory =
          new ThreadFactory() {
            ...
            public Thread newThread(Runnable r) {
              final Thread thread =
                  new Thread(r, "message-handler-" + threadNumber.getAndIncrement());
