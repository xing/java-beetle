...
public abstract class MessageHandler {
  ...
  public abstract Callable<HandlerResponse> doProcess(
      Envelope envelope, AMQP.BasicProperties properties, byte[] body);

  public Callable<HandlerResponse> process(
      Envelope envelope, AMQP.BasicProperties properties, byte[] body) {
    ...
    return new Callable<HandlerResponse>() {
      ...
      public HandlerResponse call() throws Exception {
        ...
        if (channels.isEmpty()) {
          // happens when pause() call was executed in between calling doProcess() and acquiring the
          // read lock.
          // there's nothing to do that case, because the broker already knows that messages in
          // delivery won't be processed by this consumer.
