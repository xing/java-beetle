...
public class ClientBuilderTest {
  ...
  public void testAddBrokerWorksFullyDefined() throws Exception {
    ...
    assertEquals(
        "amqp://foo:bar@0.0.0.0:5555/baz",
        client.getBrokerUris().toArray(new URI[1])[0].toString());
  }
  ...
  public void testAddBrokerWorksWithAmqpUri() throws Exception {
    ...
    assertEquals(
        "amqp://somebody:pass@example.org:1234/",
        client.getBrokerUris().toArray(new URI[1])[0].toString());
  }
  ...
  public void testAddBrokerWorksWithHostAndPort() throws Exception {
    ...
    assertEquals(
        "amqp://guest:guest@example.org:5555/",
        client.getBrokerUris().toArray(new URI[1])[0].toString());
  }
  ...
  public void testAddBrokerWorksWithVirtualHostNotStartingWithSlash() throws Exception {
    ...
    assertEquals(
        "amqp://foo:bar@0.0.0.0:5555/noslash",
        client.getBrokerUris().toArray(new URI[1])[0].toString());
  }

  @Test
  public void testSetsDeduplicationStore() throws Exception {
    // Using 0.0.0.0 because 127.0.0.1 is the automatic default and we want to test it is actually
    // being set.
