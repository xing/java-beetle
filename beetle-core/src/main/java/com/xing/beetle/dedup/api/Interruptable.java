...
public class Interruptable<M> implements MessageListener<M> {
  ...
  public boolean handleFailed(Throwable exception, int attempt) {
    try {
      ...
      System.out.println("HANDLE FAILED CALLED ON "+ this.hashCode() +" thread "+ current.getId() + " delegate "+ delegate.hashCode());
  ...
  public void onDropped(M message, String reason) {
    try {
      current = Thread.currentThread();
      System.out.println("ON DROPPED CALLED ON "+ this.hashCode() +" thread "+ current.getId()+ " delegate "+ delegate.hashCode());
  ...
  public void onMessage(M message) throws Throwable {
    try {
      current = Thread.currentThread();
      System.out.println("ON MESSAGE CALLED ON "+ this.hashCode() +" thread "+ current.getId()+ " delegate "+ delegate.hashCode());
