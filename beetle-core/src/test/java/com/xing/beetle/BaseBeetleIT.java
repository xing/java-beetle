...
class BaseBeetleIT {
  ...
  static Connection createConnection(
      BeetleConnectionFactory factory, RabbitMQContainer[] containers) throws Exception {
    Address[] addresses =
        Stream.of(containers).map(BaseBeetleIT::addressOf).toArray(Address[]::new);
    return factory.newConnection(addresses);
  }

  static Stream<Connection> createConnections(
      ConnectionFactory factory, RabbitMQContainer[] containers) throws Exception {
