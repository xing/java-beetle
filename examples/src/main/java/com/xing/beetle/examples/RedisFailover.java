...
public class RedisFailover {
    ...
    private void run(String redisProcessPath) throws URISyntaxException {
        ...
        final ProcessBuilder pbMaster = new ProcessBuilder().command(redisProcessPath, "--port", "6379");
        pbMaster.redirectErrorStream(true);

        final ProcessBuilder pbSlave = new ProcessBuilder().command(redisProcessPath, "--port", "6380", "--slaveof", "127.0.0.1", "6379");
        pbSlave.redirectErrorStream(true);

        try {
            final Process redisMaster = pbMaster.start();
            new AsyncStreamCopier(redisMaster.getInputStream(), new LoggerOutputStream(LoggerFactory.getLogger("redis-master")), "redis-master").start();
            final Process redisSlave = pbSlave.start();
            new AsyncStreamCopier(redisSlave.getInputStream(), new LoggerOutputStream(LoggerFactory.getLogger("redis-slave")), "redis-slave").start();
